...
void deinit(tree & t) {
  if (t!=NULL) {
    ...
    delete [] t->p->nome;
    delete [] t->p->cognome;
    delete [] t->p->indirizzo;
    // Mi ero dimenticato di deallocare t->p
    delete t->p;
    delete t;
  }
}
...
tree  modifica_indirizzo (tree  t, char * cognome,char * indirizzo)
{
  ...
  if (tmp!=NULL) {
				delete [] tmp->p->indirizzo;
				int l = strlen(indirizzo)+1;
				tmp->p->indirizzo = new char[l];
    strncpy(tmp->p->indirizzo,indirizzo,l);
		}
  return tmp;
}
