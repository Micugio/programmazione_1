...
void calcola_aux(Stack * & s, Stack * & res, int count[]) {
    if (isEmpty(s))
        return;
    int v = pop(s);
    count[v]++;
    calcola_aux(s, res, count);
    push(s, v);
    push(res, count[v]);
    push(res, v);
    count[v]--;
...
void calcola_auxa(Stack * & s, Stack * & res, int count[]) {
    if (isEmpty(s))
        return;
    int v = pop(s);
    count[v]++;
    // MR: cosi' gli elementi sono inseriti in modo rovesciato!
    push(res, count[v]);
    push(res, v);
    calcola_auxa(s, res, count);
    push(s, v);
}
